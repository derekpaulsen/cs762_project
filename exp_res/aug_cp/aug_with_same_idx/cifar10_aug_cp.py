...
def _slice_dataset(dataset, percent):
    if percent == 1.0:
        ...
        return Subset(dataset, indexes),indexes

def _slice_dataset_idx(dataset, indexes):
...
def load_cifar10(norm_train_percent, syn_train_percent, syn_2_train_percent, rotation_train_percent, v_flip_train_percent, device='cuda'):
    ...
    if syn_2_train_percent > 0:
        ...
        trainset,_ = _slice_dataset(SyntheticCIFAR10('./data/synthetic_cifar10_2/', train=True, transform=train_tfms), syn_2_train_percent)
        train_datasets.append(trainset)

    if syn_train_percent > 0:
        # prompt : "{category}"
        trainset,_ = _slice_dataset(SyntheticCIFAR10('./data/synthetic_cifar10/', train=True, transform=train_tfms), syn_train_percent)
        train_datasets.append(trainset)

    if norm_train_percent > 0:
        # the original cifar 10 dataset
        trainset,idx = _slice_dataset(datasets.CIFAR10('./data', train=True, transform=train_tfms, download=True), norm_train_percent)
        train_datasets.append(trainset)

    # norm_train_percent should be larger than 0 and provides idx first
    if rotation_train_percent > 0:
        trainset = _slice_dataset_idx(datasets.CIFAR10('./data/rotation', train=True, transform=train_tfms_rotation, download=True), idx)
        train_datasets.append(trainset)

    if v_flip_train_percent > 0:
        trainset = _slice_dataset_idx(datasets.CIFAR10('./data/v_flip', train=True, transform=train_tfms_v_flip, download=True), idx)
        train_datasets.append(trainset)
